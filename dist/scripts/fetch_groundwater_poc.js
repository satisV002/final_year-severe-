...
async function fetchPincode(village, district) {
    const lookup = village || district;
    if (!lookup)
        return null;
    try {
        const res = await axios_1.default.get(`${POSTAL_API}/${encodeURIComponent(lookup.trim())}`, { timeout: 8000 });
        const offices = res.data?.[0]?.PostOffice ?? [];
        if (!offices.length) {
            // fallback: search by district
            if (village) {
                const r2 = await axios_1.default.get(`${POSTAL_API}/${encodeURIComponent(district.trim())}`, { timeout: 8000 });
                const d_offices = r2.data?.[0]?.PostOffice ?? [];
                if (d_offices.length)
                    return d_offices[0].Pincode ?? null;
            }
            return null;
        }
        // prefer matching district
        const matched = offices.find((o) => o.District?.toLowerCase() === district.toLowerCase());
...
async function fetchGroundwaterData() {
    ...
    try {
        const response = await axios_1.default.post(WRIS_BASE, null, { params, timeout: 30000 });
...
//# sourceMappingURL=fetch_groundwater_poc.js.map